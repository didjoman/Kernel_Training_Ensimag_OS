...
void init_clock()
{
	// Send a command to the Clock, to say we will set a new frequency value
        outb(0x34, PORT_CLOCK_CMD);
	// Send the 8 Less Significant Bits of the freq value
        outb((QUARTZ / CLOCKFREQ) >> 8, PORT_CLOCK_DATA); // %256 = >>8
	// Send the 8 Most Significant bits of the freq value
        outb((QUARTZ / CLOCKFREQ) & 0xFF, PORT_CLOCK_DATA);	
}
...
void tic_PIT(void)
{
	...
	outb(0x20, PORT_INTERR_CONTROL_CMD);
	// update the uptime
	uptime_ms += 20;
...
void init_traitant_IT(int32_t num_IT, void (*traitant)(void))
{
	 ...
	 */
	uint32_t* table_addr = (uint32_t*) IDT_ADDR;
...
void masque_IRQ(uint32_t num_IRQ, bool masque)
{
	...
	uint8_t mask_val = inb(PORT_IRQ_DATA);
